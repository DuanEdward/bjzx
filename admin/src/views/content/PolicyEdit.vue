    ...
    <div class="page-header">
      <div class="header-left">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/content/policy' }">政策法规管理</el-breadcrumb-item>
          <el-breadcrumb-item>{{ isEdit ? '编辑政策法规' : '新增政策法规' }}</el-breadcrumb-item>
        </el-breadcrumb>
        <h2>{{ isEdit ? '编辑政策法规' : '新增政策法规' }}</h2>
      </div>
...
const formData = reactive({
  ...
  coverImage: '',
  author: '',
  categoryId: 4, // 政策法规分类ID，需要根据实际情况调整
  status: 0,
  isTop: 0,
  publishedAt: ''
})
...
const fetchPolicyDetail = async () => {
  if (!isEdit.value || !policyId.value) return

  try {
    const result = await request.get(`/news/${policyId.value}`)
    ...
    Object.assign(formData, {
      ...
      categoryId: policy.categoryId || 4,
      status: policy.status ?? 0,
      isTop: policy.isTop ?? 0,
    ...
    if (policy.coverImage) {
      fileList.value = [{
        name: policy.coverImage.split('/').pop() || '封面图片',
        url: policy.coverImage
      } as UploadFile]
    } else {
      fileList.value = []
    }
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await request.put(`/news/${policyId.value}`, formData)
    } else {
