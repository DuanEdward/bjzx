  ...
  <div class="contact-page">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="message" label="留言内容" min-width="300" show-overflow-tooltip />
...
const fetchContactList = async () => {
  try {
    ...
    const result = await request.get('/contact/list', { params })
    if (result.data) {
...
const handleView = async (row: Contact) => {
  ...
  try {
    const result = await request.get(`/contact/${row.id}`)
    if (result.data) {
...
// 编辑
const handleEdit = async (row: Contact) => {
  isEdit.value = true
  currentId.value = row.id
  try {
    const result = await request.get(`/contact/${row.id}`)
...
const handleSave = async () => {
  ...
  try {
    ...
    if (currentId.value) {
      await request.put(`/contact/${currentId.value}`, formData)
    }
